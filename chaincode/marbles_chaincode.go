...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	} else if function == "create_invoice" {									//create a new marble
...
func (t *SimpleChaincode) init_marble(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	//build the marble json string manually
	str := `{"name": "` + name + `", "color": "` + color + `", "size": ` + strconv.Itoa(size) + `, "user": "` + user + `", "vendorid": "` + VendorID + `", "customerid": "` + CustomerID + `", "invoicenumber": "` + InvoiceNumber + `", "invoiceamount": ` + InvoiceAmount + `, "currency": "` + Currency + `", "material": "` + Material + `","quantity":` + Quantity + `,"traderid":"` + TraderID + `","paymentdate":"` + PaymentDate + `","Status":"` + Status + `","newpaymentdate":"` + NewPaymentDate + `"}`
	err = stub.PutState(name, []byte(str))									//store marble with id as key
	if err != nil {
		return nil, err
	}
...
func (t *SimpleChaincode) create_invoice(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	if res.InvoiceNumber == InvoiceNumber{
		...
		fmt.Println(res);
		return nil, errors.New("This invoice arleady exists")				//all stop a marble by this name exists
	}
	
	//build the invoice json string manually
	//str := `{"name": "` + VendorID + `", "color": "` + color + `", "size": ` + strconv.Itoa(size) + `, "user": "` + user + `","vendorid:abc"}`
	//, , , , , , , , , , 
	...
	err = stub.PutState(VendorID, []byte(str))									//store marble with id as key
	if err != nil {
		return nil, err
	}
	...
	//append
	invoiceIndex = append(invoiceIndex, VendorID)									//add marble name to index list
	fmt.Println("! marble index: ", invoiceIndex)
	jsonAsBytes, _ := json.Marshal(invoiceIndex)
	err = stub.PutState(invoiceIndexStr, jsonAsBytes)						//store name of marble
