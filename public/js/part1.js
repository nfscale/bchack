...
$(document).on('ready', function() {
	...
	$('#submit').click(function(){
		...
		var obj = 	{
						...
						quantity : "10",
						traderid : "TRADER1",
						paymentdate : "2016/12/22",
						status : "Approved",
						newpaymentdate : "2016/12/23",
						v: 1
					};
	...
	function showHomePanel(){
		...
		setTimeout(function(){
			$('#user1wrap').html('');											//reset the panel
			$('#user2wrap').html('');
			ws.send(JSON.stringify({type: 'get_invoice', v: 1}));						//need to wait a bit
			ws.send(JSON.stringify({type: 'chainstats', v: 1}));
		}, 1000);
	}
...
function connect_to_server(){
	...
	function onOpen(evt){
		...
		//ws.send(JSON.stringify({type: 'create_invoice', v:1}));
		//console.log('after create invoice');
		
		ws.send(JSON.stringify({type: 'chainstats', v:1}));
		//ws.send(JSON.stringify({type: 'get_invoice', v:1}));
	}
...
function build_ball(data){
	...
	if(!$('#' + data.name).length){								//only populate if it doesn't exists
		...
		html += '['+data.invoicenumber+'] <span id="' + data.name + '" class="fa fa-circle ' + size + ' ball ' + colorClass + ' title="' + data.name + '" user="' + data.user + '"></span>';
