	...
	if(data.v === 1){																						//only look at messages for part 1
		if(data.type == 'create'){
			...
			if(data.name && data.color && data.size && data.user){
				...
				chaincode.invoke.init_marble([data.name, data.color, data.size, data.user, data.customerid, data.invoicenumber, data.invoiceamount, data.currency, data.material, data.quantity, data.traderid, data.paymentdate, data.status, data.newpaymentdate], cb_invoked);	//create a new marble
			}
			//chaincode.invoke.create_invoice(['ven001', 'cust001', 'inv001', '1000', 'INR', 'material', '14', 'trd01', '12/12/2016', 'Appoved', '12/12/2016'], cb_invoked);
		}
		else if(data.type == 'get'){
			console.log('get marbles msg');
			chaincode.query.read(['_marbleindex'], cb_got_index);
		}
		else if(data.type == 'get_invoice'){
			console.log('get get_invoice msg');
			chaincode.query.read(['_invoiceindex'], cb_got_index);
		}
		...
		}else if(data.type == 'create_invoice'){
			console.log('into create invoice as server side');
			//if(data.name){
				chaincode.invoke.create_invoice(['ven001', 'cust001', 'inv001', '1000', 'INR', 'material', '14', 'trd01', '12/12/2016', 'Appoved', '12/12/2016'], cb_invoked);
			//}
			console.log('into create invoice as server side finish');
		}
	}
